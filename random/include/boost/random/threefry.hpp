...
*
* generator       length of cycle    approx. memory req.   approx. speed compared to fastest
* threefry13_32   2^259              26*sizeof(in32_t)    104%
* threefry20_32   2^259              26*sizeof(in32_t)     77%
* threefry13_64   2^258              26*sizeof(in32_t)     59%
* threefry20_64   2^258              26*sizeof(in32_t)     42%
* (tests compared agains boost::mt11213b)
*
*
* properties for parallel computing:
* O(1) discard which allows for fast parallel leapfrog schemes.
* 256 bit seed, engines with different seeds generate independent mutually non-overlapping streams.
* Passes the TestU01 BigCrush test
* efficient: it's fast and uses little memory.
*
* algorithm:
* Threefish is a symmetric-key block cipher that can be used to encrypt and 
* decrypt messages using the same key. Counter based random engines like this threefry 
* engine use encryption to encrypt a simple counter -in this case a 256 bit integer-. 
* The encryption uses a key, and that same key can also be used to decrypt the message. 
*
* In the threefry random engine a 256 bit key is uses that can be changed via the seed().
*
...
* counter is incremented and again encrypted to generate a new encrypted message. 
* Small changes in the counter -or the key- result in big changes the encrypted output 
* messages. 
* The fact that it's very difficult to decrypt the message without knowing the key makes
* it sound to use a simple counter as a message source. To be able to predict the 
* random output of the engine one would need to solve a task that is similar to decrypt 
* the message and recover the counter (after that the counter is easy to predict).
* 
* Using an encrypted counter makes discard() very efficient. We simply advance the counter 
* as much as we want and then encrypt the new counter.
* Another useful property is that the encryption is invertible (you can decrypt the 
* message and recover the counter if you know the key). This property ensures a 1-1 
* mapping between the counters and output messages, there are exactly as many 
* different output messages are there are counter states.
...
* encryption/decryption process, if it weren't then recovering the key would be easy.
* The big unpredictable effect of changes in the key makes it plausible that different 
* keys give independent random streams. If there was some dependency than that would 
* allow security researches to attach the Threefish encryption scheme. 
* 
* The Threefry random engines uses a stripped down version of the Threefish encryption
* cipher. Threefish uses 72 encryption round, and Threefry uses 13 or 20 round. This 
* reduction in rounds was done to speed up the random engine. 13 rounds were picked 
* because that's the minimal number of rounds that still allow the engine to pass the 
* stringent BigCrush test. The 20 rounds version is slower but has 7 extra rounds as 
* a safety margin. 
*
*
* BigCrush test:
* The quality of the random number generates has been tested by running the extensive 
* BigCrush test from the TestU01 framework. The BigCrush test consist of a suite of
* 160 statistical test and takes 3 hours to complete. 
* 
* - http://www.iro.umontreal.ca/~simardr/testu01/tu01.html
* - http://www.iro.umontreal.ca/~lecuyer/myftp/papers/testu01.pdf
*
* 
* Threefry Random Engine Reference:
* @blockquote
